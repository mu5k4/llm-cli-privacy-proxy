...
- On September 2, 2026, removed the `/responses` tool-type allowlist so Codex-native tool definitions are no longer blocked by proxy-local name gating. The stack was rebuilt and the full regression suite passed after the change.
- On September 2, 2026, committed the Codex `/responses` compatibility, performance, tracing, and docs updates on branch `fix/codex-responses-compat-perf` in commit `b4fafa3`.
...
- On September 2, 2026, completed a `/responses` compatibility and performance hardening pass for Codex traffic: allowed bounded `client_metadata`, removed tool type name gating, added JSON-schema `body.text.format` support, added structured timing traces, and optimized large benign payloads to skip expensive Nosey Parker scans. Regression coverage and live traced requests verified the changes.
